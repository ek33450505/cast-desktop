...
function TokenSpendInline() {
  ...
  if (!data) return <div className="p-6 text-[var(--content-muted)]">No token spend data available.</div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bento-card p-5">
          ...
          {budget?.daily_limit != null && (
            <div className="text-xs text-[var(--content-secondary)] mt-1">
      ...
      {daily.length > 0 && (
        <div className="bento-card p-6">
          <h3 className="text-sm font-semibold text-[var(--content-primary)] mb-4">Daily Token Spend</h3>
          <ResponsiveContainer width="100%" height={280}>
            <AreaChart data={daily}>
              <CartesianGrid strokeDasharray="3 3" stroke={colors.gridStroke} />
              <XAxis dataKey="date" tick={{ fontSize: 10, fill: colors.axisTick }} tickFormatter={d => d.slice(5)} />
              <YAxis tick={{ fontSize: 10, fill: colors.axisTick }} tickFormatter={v => `$${Number(v).toFixed(2)}`} />
              <Tooltip contentStyle={tooltipStyle} formatter={(v: number) => [`$${v.toFixed(4)}`, 'Cost']} />
              <Area type="monotone" dataKey="costUsd" stroke={colors.accent} fill={colors.accentMuted} strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      {(totals.cacheCreationTokens > 0 || totals.cacheReadTokens > 0) && (
        <div className="bento-card p-5">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <span className="text-[var(--content-muted)]">Cache Creation:</span>
              <span className="ml-2 font-mono text-[var(--content-secondary)]">{formatTokens(totals.cacheCreationTokens)}</span>
            </div>
            <div>
              <span className="text-[var(--content-muted)]">Cache Reads:</span>
              <span className="ml-2 font-mono text-[var(--content-secondary)]">{formatTokens(totals.cacheReadTokens)}</span>
            </div>
          </div>
        </div>
      )}
...
function AgentScorecard() {
  ...
  if (error) {
    return <div className="bento-card p-6 text-[var(--status-error)] text-sm">{error instanceof Error ? error.message : 'Failed to load'}</div>
  }
  ...
  return (
    <div className="bento-card overflow-hidden">
      <div className="px-6 py-4 border-b border-[var(--stroke-regular)]">
        <h2 className="text-sm font-semibold text-[var(--content-secondary)] uppercase tracking-wider">Agent Scorecard</h2>
        <p className="text-xs text-[var(--content-muted)] mt-1">Per-agent success rate, blocked count, and avg cost from cast.db</p>
      </div>
      <div className="overflow-x-auto" role="region" aria-label="Agent scorecard table — per-agent runs, success rate, blocked count, and average cost">
        <table className="w-full text-sm min-w-[340px] md:min-w-[560px]">
          <thead>
            <tr className="border-b border-[var(--stroke-regular)]">
              <th scope="col" className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[var(--content-muted)]">Agent</th>
              <th scope="col" className="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--content-muted)]">Runs</th>
              <th scope="col" className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[var(--content-muted)] min-w-[120px] md:min-w-[160px]">Success Rate</th>
              {/* Hidden on mobile — shown at md+ */}
              <th scope="col" className="hidden md:table-cell px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--content-muted)]">Blocked</th>
              <th scope="col" className="hidden md:table-cell px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--content-muted)]">Avg Cost</th>
            </tr>
          </thead>
          <tbody>
            {agents.map(a => {
              const underperformer = a.success_rate < 70 && a.blocked_count > 5
              // Success rate bar color: semantic status colors (these are true status signals)
              const barColor = a.success_rate >= 80 ? colors.success : a.success_rate >= 70 ? colors.warning : colors.error
              const barPct = Math.max(0, Math.min(100, a.success_rate))
              return (
                <tr key={a.name} className="border-b border-[var(--stroke-regular)] hover:bg-[var(--system-elevated)] transition-colors">
                  <td className="px-6 py-3 font-medium text-[var(--content-primary)]">
                  ...
                  <td className="px-6 py-3 text-right text-[var(--content-secondary)] tabular-nums">{a.runs}</td>
                  <td className="px-6 py-3">
                    <div className="flex items-center gap-2">
                      <div
                        className="flex-1 h-2 rounded-full bg-[var(--system-panel)] overflow-hidden min-w-[60px] md:min-w-[80px]"
                        role="progressbar"
                  ...
                  {/* Hidden on mobile */}
                  <td className="hidden md:table-cell px-6 py-3 text-right tabular-nums" style={{ color: a.blocked_count > 5 ? colors.error : 'var(--content-secondary)' }}>
                    {a.blocked_count}
                  </td>
                  <td className="hidden md:table-cell px-6 py-3 text-right text-[var(--content-muted)] tabular-nums font-mono text-xs">
      ...
      <div className="px-6 py-2 border-t border-[var(--stroke-regular)] text-xs text-[var(--content-muted)] flex items-center gap-1" aria-label="Legend: triangle icon indicates underperformer">
        <AlertTriangle className="w-3 h-3" style={{ color: colors.error }} aria-hidden="true" />
...
function DelegationSavingsPanel({ savings }: { savings: DelegationSavings }) {
  ...
  return (
    <div
      ...
      style={{
        background: `repeating-linear-gradient(0deg, rgba(0,0,0,0.08) 0px, rgba(0,0,0,0.08) 1px, transparent 1px, transparent 3px), var(--system-panel)`,
        border: `2px solid var(--stroke-regular)`,
      }}
    >
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          {/* Section header: label, not data — use content-secondary */}
          <h2 className="text-xs font-semibold text-[var(--content-secondary)] uppercase tracking-wider" style={PIXEL_FONT}>
            DELEGATION SAVINGS
          </h2>
          <p className="text-xs text-[var(--content-muted)]">Haiku dispatch vs all-sonnet baseline</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-5">
          {/* Hero stat: the saved USD amount — this is the one number that matters */}
          <div>
            <div className="text-xs text-[var(--content-muted)] mb-1">SAVED VS ALL-SONNET</div>
            <div style={{ ...PIXEL_FONT, fontSize: 14, color: colors.accent, lineHeight: 2 }}>
              ${savings.savedUSD.toFixed(4)}
            </div>
            {/* Supporting stats: neutral — not hero, not status */}
            <div className="text-xs text-[var(--content-muted)] mt-0.5">
              actual ${savings.actualCostUSD.toFixed(4)} · baseline ${savings.hypotheticalSonnetCostUSD.toFixed(4)}
            </div>
          </div>

          {/* Haiku utilization — model-tier semantic color */}
          <div>
            <div className="text-xs text-[var(--content-muted)] mb-2">HAIKU UTIL</div>
            <div className="flex items-center gap-3">
              <PixelBar pct={savings.haikuUtilizationPct} color={colors.haiku} bg={colors.gridStroke} />
              <span style={{ ...PIXEL_FONT, fontSize: 8, color: colors.haiku }}>
        ...
        {/* Right: Per-model dispatch chips — semantic model-tier colors */}
        <div>
          <div className="text-xs text-[var(--content-muted)] mb-3">MODEL DISPATCH SPLIT</div>
          <div className="space-y-3">
            {[
              { label: 'HAIKU', count: savings.dispatches.haiku, pct: haikuPct, color: colors.haiku },
              { label: 'SONNET', count: savings.dispatches.sonnet, pct: sonnetPct, color: colors.sonnet },
              { label: 'OPUS', count: savings.dispatches.opus, pct: opusPct, color: colors.opus },
            ].map(({ label, count, pct, color }) => (
              <div key={label} className="flex items-center gap-3">
                <span
                  className="px-2 py-1 rounded"
                  style={{ ...PIXEL_FONT, fontSize: 6, color, background: `${color}25`, minWidth: 52, textAlign: 'center' }}
                >
                  {label}
                </span>
                <div className="flex-1 h-1.5 rounded-full bg-[var(--system-elevated)] overflow-hidden">
                  <div
                    className="h-full rounded-full transition-all duration-700"
                    style={{ width: `${pct}%`, backgroundColor: color }}
                  />
                </div>
                <span className="text-xs text-[var(--content-muted)] w-8 text-right">{count}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          {/* Section header: label — content-secondary */}
          <h2 className="text-xs font-semibold text-[var(--content-secondary)] uppercase tracking-wider" style={PIXEL_FONT}>
            CACHE EFFICIENCY
          </h2>
          <p className="text-xs text-[var(--content-muted)]">Prompt cache creation vs read ratio</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-5">
          {/* Hero stat: hit ratio — the primary measurement for this section */}
          <div>
            <div className="text-xs text-[var(--content-muted)] mb-1">HIT RATIO</div>
            <div style={{ ...PIXEL_FONT, fontSize: 14, color: colors.accent, lineHeight: 2 }}>
              {hitRatio}%
            </div>
            <div className="text-xs text-[var(--content-muted)] mt-0.5">
          ...
          {/* Supporting stat: cache savings — measurement, not status */}
          <div>
            <div className="text-xs text-[var(--content-muted)] mb-1">CACHE SAVINGS</div>
            <div style={{ ...PIXEL_FONT, fontSize: 11, color: colors.accentMuted, lineHeight: 2 }} className="text-[var(--content-secondary)]">
              <span className="text-[var(--content-primary)]">{formatCost(cacheSavingsUSD)}</span>
            </div>
            <div className="text-xs text-[var(--content-muted)] mt-0.5">vs paying full input rate for cache reads</div>
          </div>
        </div>

        <div>
          <div className="text-xs text-[var(--content-muted)] mb-3">TOKEN BREAKDOWN</div>
          <div className="space-y-3">
            {[
              { label: 'CACHE WRITE', count: totalCacheCreationTokens, pct: creationPct, color: colors.haiku },
              { label: 'CACHE READ', count: totalCacheReadTokens, pct: readPct, color: colors.sonnet },
            ].map(({ label, count, pct, color }) => (
              <div key={label} className="flex items-center gap-3">
                <span
                  className="px-2 py-1 rounded"
                  style={{ ...PIXEL_FONT, fontSize: 6, color, background: `${color}25`, minWidth: 72, textAlign: 'center' }}
                >
                  {label}
                </span>
                <div className="flex-1 h-1.5 rounded-full bg-[var(--system-elevated)] overflow-hidden">
                ...
                <span className="text-xs text-[var(--content-muted)] w-14 text-right">{formatTokens(count)}</span>
              </div>
            ))}
          </div>
        </div>
...
export default function AnalyticsView() {
  ...
  // Model color: use chart colors from hook (model-tier semantic)
  ...
  return (
    <div className="space-y-6 animate-in">
      ...
      {/* Tab bar */}
      <div className="flex gap-1 border-b border-[var(--stroke-regular)]">
      ...
      {data.sessionsByDay.length > 1 && (
        <div className="bento-card p-6">
          <h2 className="text-sm font-semibold text-[var(--content-secondary)] uppercase tracking-wider mb-4">Daily Token Usage (Last 90 Days)</h2>
            ...
            <AreaChart data={data.sessionsByDay}>
              <defs>
                ...
                <linearGradient id="gradientMuted" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={colors.sonnet} stopOpacity={0.2} />
                  <stop offset="95%" stopColor={colors.sonnet} stopOpacity={0} />
                </linearGradient>
              </defs>
              <CartesianGrid strokeDasharray="3 3" stroke={colors.gridStroke} />
              <XAxis
              ...
              <Legend iconType="circle" wrapperStyle={{ fontSize: 12, color: colors.axisTick }} />
              {/* Input = hero series (accent); Output = secondary (sonnet/muted) */}
              <Area type="monotone" dataKey="inputTokens" name="Input" stroke={colors.accent} fill="url(#gradientAccent)" strokeWidth={2} />
              <Area type="monotone" dataKey="outputTokens" name="Output" stroke={colors.sonnet} fill="url(#gradientMuted)" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {data.toolUsage.length > 0 && (
          <div className="bento-card p-6">
            <h2 className="text-sm font-semibold text-[var(--content-secondary)] uppercase tracking-wider mb-4">Top Tools by Usage</h2>
            ...
            <ResponsiveContainer width="100%" height={Math.max(250, data.toolUsage.slice(0, 10).length * 32)}>
              <BarChart data={data.toolUsage.slice(0, 10)} layout="vertical">
                ...
                <YAxis
                  ...
                  dataKey="tool"
                  width={120}
                  tick={{ fill: colors.tooltipText, fontSize: 11, fontFamily: 'Geist Mono, monospace' }}
                  axisLine={{ stroke: colors.gridStroke }}
                />
                <Tooltip contentStyle={tooltipStyle} />
                {/* Single series — accent color is appropriate as the primary data */}
                <Bar dataKey="count" name="Calls" fill={colors.accent} radius={[0, 4, 4, 0]} />
              </BarChart>
            </ResponsiveContainer>
            </div>
          </div>
        ...
        {data.modelBreakdown.length > 0 && (
          <div className="bento-card p-6">
            <h2 className="text-sm font-semibold text-[var(--content-secondary)] uppercase tracking-wider mb-4">Cost by Model</h2>
              ...
              {data.modelBreakdown.filter(e => e.cost > 0).map((entry) => {
                ...
                return (
                  <div key={entry.model} className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-[var(--content-secondary)]">{getModelShort(entry.model)}</span>
                    {/* Cost amounts are measurements, not status — use accent for the primary value */}
                    <span className="text-[var(--accent)] font-medium tabular-nums">{formatCost(entry.cost)}</span>
                    <span className="text-[var(--content-muted)]">({pct}%)</span>
                  </div>
                )
              })}
            </div>
      ...
      {data.sessionsByDay.length > 1 && (
        <div className="bento-card p-6">
          <h2 className="text-sm font-semibold text-[var(--content-secondary)] uppercase tracking-wider mb-4">
            Daily Token Burn (Last 30 Days)
          </h2>
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={data.sessionsByDay.slice(-30)}>
              ...
              <Tooltip contentStyle={tooltipStyle} formatter={(v: number) => [formatTokens(v)]} />
              <Legend iconType="circle" wrapperStyle={{ fontSize: 12, color: colors.axisTick }} />
              <Bar dataKey="inputTokens" name="Input" stackId="a" fill={colors.accent} opacity={0.85} />
              <Bar dataKey="outputTokens" name="Output" stackId="a" fill={colors.sonnet} opacity={0.85} radius={[3, 3, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
          </div>
        </div>
      ...
      {promptActivityData.length > 0 && (
        <div className="bento-card p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-[var(--content-secondary)] uppercase tracking-wider">Prompt Activity (Last 14 Days)</h2>
            <span className="text-xs text-[var(--content-secondary)] tabular-nums">
          ...
          <div className="min-h-[160px]" role="img" aria-label="Bar chart showing daily user prompt submissions over the last 14 days">
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={promptActivityData}>
                ...
                />
                <Tooltip contentStyle={tooltipStyle} />
                {/* Prompt activity — use accent-muted (restrained) as this is secondary to token charts */}
                <Bar dataKey="count" name="Prompts" fill={colors.accentMuted} stroke={colors.accent} strokeWidth={1} radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      {sortedProjects.length > 0 && (
        <div className="bento-card overflow-hidden">
          <div className="px-6 py-4 border-b border-[var(--stroke-regular)]">
            <h2 className="text-sm font-semibold text-[var(--content-secondary)] uppercase tracking-wider">Cost by Project</h2>
          </div>
          <div className="overflow-x-auto" role="region" aria-label="Cost by project — sortable table">
            <table className="w-full text-sm min-w-[480px]">
              <thead>
                <tr className="border-b border-[var(--stroke-regular)]">
                  <th scope="col" onClick={() => toggleSort('project')} aria-sort={sortKey === 'project' ? (sortDir === 'asc' ? 'ascending' : 'descending') : 'none'} className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[var(--content-muted)] cursor-pointer hover:text-[var(--accent)] transition-colors">
                    Project{sortIndicator('project')}
                  </th>
                  <th scope="col" onClick={() => toggleSort('sessions')} aria-sort={sortKey === 'sessions' ? (sortDir === 'asc' ? 'ascending' : 'descending') : 'none'} className="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--content-muted)] cursor-pointer hover:text-[var(--accent)] transition-colors">
                    Sessions{sortIndicator('sessions')}
                  </th>
                  <th scope="col" onClick={() => toggleSort('tokens')} aria-sort={sortKey === 'tokens' ? (sortDir === 'asc' ? 'ascending' : 'descending') : 'none'} className="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--content-muted)] cursor-pointer hover:text-[var(--accent)] transition-colors">
                    Tokens{sortIndicator('tokens')}
                  </th>
                  <th scope="col" onClick={() => toggleSort('cost')} aria-sort={sortKey === 'cost' ? (sortDir === 'asc' ? 'ascending' : 'descending') : 'none'} className="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--content-muted)] cursor-pointer hover:text-[var(--accent)] transition-colors">
              ...
              <tbody>
                {sortedProjects.map(({ project, sessions, tokens, cost }) => (
                  <tr key={project} className="border-b border-[var(--stroke-regular)] hover:bg-[var(--system-elevated)] transition-colors">
                    <td className="px-6 py-3 font-semibold text-[var(--content-primary)]">{project}</td>
                    <td className="px-6 py-3 text-right text-[var(--content-secondary)] tabular-nums">{sessions}</td>
                    <td className="px-6 py-3 text-right text-[var(--content-secondary)] tabular-nums">{formatTokens(tokens)}</td>
                    {/* Cost is the hero stat in this table — use accent */}
      ...
      <div className="space-y-6 mt-6">
        <h2 className="text-sm font-semibold text-[var(--content-secondary)] uppercase tracking-wider">CAST Observability</h2>
