    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="bento-card p-6 space-y-3">
          <div className="h-4 w-24 rounded animate-pulse bg-[var(--system-elevated)]" />
          <div className="h-3 w-16 rounded animate-pulse bg-[var(--system-elevated)]" />
          <div className="h-3 w-full rounded animate-pulse bg-[var(--system-elevated)]" />
        </div>
      ))}
    </div>
  )
...
function RoutingIntelSection() {
  ...
  return (
    <section>
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center gap-2 text-sm font-semibold text-[var(--content-secondary)] hover:text-[var(--content-primary)] transition-colors mb-2"
        aria-expanded={open}
      >
        {open
          ...
          {tab === 'injection' && (
            <div className="overflow-x-auto max-h-64 overflow-y-auto">
              <table className="w-full text-xs">
                <thead className="sticky top-0 bg-[var(--system-panel)] z-10">
                  <tr className="border-b border-[var(--border)]">
                    <th className="text-left px-3 py-2 font-medium text-[var(--content-muted)]">Injected At</th>
                    <th className="text-left px-3 py-2 font-medium text-[var(--content-muted)]">Fact ID</th>
                    <th className="text-left px-3 py-2 font-medium text-[var(--content-muted)]">Score / Prompt Hash</th>
                  </tr>
                </thead>
                <tbody>
                  {entries.length === 0 ? (
                    <tr>
                      <td colSpan={3} className="px-3 py-4 text-center text-[var(--content-muted)]">No injection log entries</td>
                    </tr>
                  ) : entries.map(e => (
                    <tr key={e.id} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--system-elevated)] transition-colors">
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-[var(--content-primary)]">Agents</h1>
      ...
      {/* ── Active Agents ── */}
      <section>
        <h2 className="text-sm font-semibold text-[var(--content-secondary)] mb-2 flex items-center gap-2">
          <Activity className="w-4 h-4" /> Active Agents
        </h2>
        {!activeAgents || activeAgents.length === 0 ? (
          <div className="bento-card p-3 text-xs text-[var(--content-muted)] text-center">
            No agents running
          </div>
        ) : (
          <div className="flex flex-wrap gap-2">
            {activeAgents.map((a) => (
              ...
              >
                ...
                <span className="text-xs font-medium text-[var(--content-primary)]">{a.agent}</span>
                {a.started_at && (
                  <span className="text-[10px] text-[var(--content-muted)]">{timeAgo(a.started_at)}</span>
                )}
              </div>
            ))}
          </div>
      ...
      <section>
        <div className="flex items-center gap-3 mb-3">
          <h2 className="text-sm font-semibold text-[var(--content-secondary)]">Agent Registry</h2>
          <div className="relative flex-1 max-w-xs">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[var(--content-muted)]" />
            <input
              type="text"
              ...
              className="w-full pl-8 pr-3 py-1.5 text-xs rounded-lg bg-[var(--system-panel)] border border-[var(--border)] text-[var(--content-primary)] placeholder:text-[var(--content-muted)] focus:outline-none focus:ring-1 focus:ring-[var(--accent)]"
            />
          </div>
        </div>
        {agentsLoading ? (
            ...
            <AnimatePresence>
              {sortedFilteredAgents.map((agent) => (
                <motion.div
                  layout
                  key={agent.name}
                  className={`bento-card p-6 space-y-2 transition-all duration-300 ${
                ...
                >
                  ...
                  <p className="text-xs text-[var(--content-muted)] line-clamp-2">{agent.description}</p>
                </motion.div>
              ))}
            </AnimatePresence>
            {sortedFilteredAgents.length === 0 && (
              <div className="col-span-full text-xs text-[var(--content-muted)] text-center py-6">
      ...
      {/* ── Scorecard Table ── */}
      <section>
        <h2 className="text-sm font-semibold text-[var(--content-secondary)] mb-2">Agent Scorecard</h2>
            ...
            <thead className="sticky top-0 bg-[var(--system-panel)] z-10">
              <tr className="border-b border-[var(--border)]">
                {([
                  ...
                  <th
                    key={key}
                    className="text-left px-3 py-2 font-medium text-[var(--content-muted)] cursor-pointer select-none hover:text-[var(--content-primary)] transition-colors"
            ...
            <tbody>
              {sortedScorecard.map((row) => (
                <tr key={row.agent} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--system-elevated)] transition-colors">
                  ...
                  <td className="px-3 py-2 tabular-nums text-[var(--content-secondary)]">{formatCost(row.avgCost)}</td>
                  <td className="px-3 py-2 text-[var(--content-muted)]">{row.lastRun ? timeAgo(row.lastRun) : '--'}</td>
                </tr>
              ))}
              {sortedScorecard.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-3 py-6 text-center text-[var(--content-muted)]">
      ...
      {/* ── Recent Runs ── */}
      <section>
        <h2 className="text-sm font-semibold text-[var(--content-secondary)] mb-2">Recent Runs</h2>
        <div className="flex flex-wrap items-center gap-2 mb-3">
          <select
            value={runAgentFilter}
            onChange={(e) => setRunAgentFilter(e.target.value)}
            aria-label="Filter by agent"
            className="text-xs px-2 py-1.5 rounded-lg bg-[var(--system-panel)] border border-[var(--border)] text-[var(--content-primary)] focus:outline-none focus:ring-1 focus:ring-[var(--accent)]"
          >
        ...
        <div className="bento-card divide-y divide-[var(--border)] max-h-96 overflow-auto">
          {filteredRuns.map((run) => (
            <div
              ...
              className="flex items-center gap-3 px-4 py-2.5 hover:bg-[var(--system-elevated)] transition-colors text-xs"
            >
              <span className="text-[var(--content-muted)] tabular-nums shrink-0 w-16">
              ...
              <span className="text-[var(--content-muted)] tabular-nums shrink-0 w-14 text-right">
                {runDuration(run)}
              </span>
          ...
          {filteredRuns.length === 0 && (
            <div className="px-4 py-6 text-xs text-[var(--content-muted)] text-center">
